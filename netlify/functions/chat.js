// Netlify Function: 資材管理DB AI チャット
// shizai-kanri-db.netlify.app 用
// 太陽光・蓄電池の設置提案・ビジネススキーム提案機能付き

const ANTHROPIC_API_URL = "https://api.anthropic.com/v1/messages";
const CLAUDE_MODEL = "claude-sonnet-4-20250514";

const SYSTEM_PROMPT = `あなたは「しろくま電力」の資材管理DBアシスタントです。
購買チームの資材検索・案件管理・比較分析をサポートし、太陽光発電・蓄電池システムの設置提案まで行います。
...
## ★★ 太陽光発電・蓄電池システム設置提案機能
ユーザーが太陽光発電や蓄電池の設置について質問した場合、以下の流れで提案すること：

### 1. 設置前の確認事項（前後関係の把握）
- **設置場所**: 屋根（折板/陸屋根/傾斜屋根）、地上（野立て）、カーポート
- **電力使用状況**: 現在の契約電力、月間使用量、ピーク需要
- **系統連系**: 高圧/低圧/特別高圧、逆潮流の有無
- **法規制**: FIT/FIP制度の適用、自家消費型か売電型か

### 2. 設備構成の提案フロー
太陽光発電システムの基本構成を以下の順序で提案：
1. **太陽光パネル（モジュール）** → DB内のPV関連資材を検索
2. **パワーコンディショナ（PCS）** → 容量マッチング
3. **架台・取付金具** → 設置場所に応じた架台
4. **ケーブル類** → DC/ACケーブル、接続箱
5. **蓄電池システム**（必要時） → 容量・用途に応じた選定
6. **監視システム** → 遠隔監視・SCADA
7. **電気工事関連** → ブレーカー、トランス、接地材

### 3. ビジネススキーム提案
- **自家消費型**: 電力コスト削減効果の試算ポイント
- **余剰売電型**: FIT/FIP単価との関係
- **PPA（電力購入契約）モデル**: 初期投資ゼロの第三者所有モデル
- **リース型**: 月額固定費での導入
- **オンサイト/オフサイト**: 設置場所と需要家の関係
- **投資回収年数の概算**: 設備費、電力単価削減分、売電収入等

### 4. 提案時の注意
- DB内の資材データに基づいた具体的な製品推奨を行う
- 価格情報がある場合は概算見積を提示する
- DB内に該当製品がない場合は「DBに未登録のため、別途調達が必要」と明示する
- 設計は概算レベルであり、詳細設計は専門業者への確認が必要と注記する
...
const TOOLS = [
  ...
  },
  {
    ...
    description: "案件名で資材を検索。特定の案件で使用された資材一覧を取得する。",
    input_schema: {
      type: "object",
      properties: {
        project_name: { type: "string", description: "案件名（部分一致）" },
  ...
  },
  {
    ...
    description: "案件一覧を取得。案件名と関連する資材数を返す。",
    input_schema: {
      type: "object",
      properties: {
        limit: { type: "integer", default: 50 }
...
async function searchByProject(projectName, limit = 50) {
  const q = encodeURIComponent(projectName);
  return sb(`materials?${SEL}&project=ilike.*${q}*&limit=${limit}&order=major_category.asc,product_name.asc`);
}

async function getProjectList(limit = 50) {
  const data = await sb("materials?select=project&order=project");
  if (Array.isArray(data)) {
    const projects = {};
    data.forEach(r => {
      const p = r.project || "案件未設定";
      projects[p] = (projects[p] || 0) + 1;
    });
    return Object.entries(projects)
      .sort((a, b) => b[1] - a[1])
      .slice(0, limit)
      .map(([k, v]) => ({ project: k, material_count: v }));
  }
  return data;
}
...
async function executeTool(name, input) {
  switch (name) {
    ...
    case "search_by_project": return searchByProject(input.project_name || "", input.limit || 50);
    case "get_project_list": return getProjectList(input.limit || 50);
